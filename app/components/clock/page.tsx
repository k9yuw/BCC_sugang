...
const Clock: React.FC = () => {
  ...
  const [musicPlayed, setMusicPlayed] = useState(false);
    ...
    const dateCheckbox = document.getElementById("dateCheckbox") as HTMLInputElement;
    const msCheckbox = document.getElementById("msCheckbox") as HTMLInputElement;
    const bgmCheckbox = document.getElementById("bgmCheckbox") as HTMLInputElement;
    ...
    clock.innerText = dateCheckbox.checked ? timeFormat + timeText : dateFormat + timeFormat + timeText;

    setDate((prevDate) => prevDate + 10);

    if (bgmCheckbox.checked && !musicPlayed && new Date(date).getSeconds() >= 53 && new Date(date).getMilliseconds() >= 0) {
  ...
  return (
    <div>
      <h2 id="clock" className={styles.timeArea}>00:00</h2>
      <label><input type="checkbox" id="dateCheckbox" /> 날짜 제거</label>
      <label><input type="checkbox" id="msCheckbox" /> 밀리초 보기</label>
      <label><input type="checkbox" id="bgmCheckbox" /> 음악 켜기</label>
